...
Conditions in `if`, `while`, and `until` require a Boolean. Pop does not convert other values into conditions:
...
Write the actual test instead, such as `if count > 0 then`.
