...
Writing only `local values = {1, 2, 3}` is rejected in 0.1.0-rc.3 because an untyped array literal does not select its own array type.
...
`Array.fill(scores, 0)` replaces every element with the same value. A compound indexed assignment performs one checked non-optional read, applies the typed operation, and writes the result; the array and index are evaluated once.

Use an inclusive numeric range to visit every position:
...
The Pop-facing Standard library does not yet provide dynamic list operations or generalized `for value in values` iteration. Arrays remain fixed-length; future growth belongs to `List<T>` rather than hidden array resizing.
