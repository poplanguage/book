...
Finally, a backtick string combines the greeting and supplied name:

```pop
print(`Hello, {name}!`)
```

Backtick interpolation accepts statically typed strings, booleans, integers, and floating-point values. Command-line arguments are always strings, even when their text looks like a number. We will learn how arrays, conditions, and types work in detail in later chapters.
