...
Checked target-type conversions trap with `NumericConversion` when a value does not fit, or when converting NaN or infinity to an integer:

```pop
local large: Int = 1_000
local compact = UInt8(large) -- traps
```

A numeric `for` whose runtime step is zero traps with `InvalidRangeStep` before its body runs.
...
Pop 0.1.0-rc.3 does not expose a source-level exception mechanism for catching these traps. A trap stops normal program execution. Validate user input and array positions before using operations whose preconditions may not hold.
